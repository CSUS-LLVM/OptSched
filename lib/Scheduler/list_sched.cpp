...
FUNC_RESULT SequentialListScheduler::FindSchedule(InstSchedule *sched, SchedRegion *rgn) {
  ...
  while (!IsSchedComplete_()) {
    ...
    SchedInstruction *inst = SequentialListScheduler::PickInst();
...
__host__ __device__
SchedInstruction *SequentialListScheduler::PickInst() const {
  SchedInstruction *inst = NULL;
  bool legalInst = false;
  while (!legalInst) {
    inst = rdyLst_->GetNextPriorityInst();
    legalInst = SequentialListScheduler::ChkInstLglty_(inst);
  }
  return inst;
}
