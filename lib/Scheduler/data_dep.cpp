...
// atoi() ignores conversion errors. strol() is safer, but slightly unwieldy. This is a wrapper for strtol().
...
SchedInstruction *DataDepGraph::CreateNode_(
  ...
  // FIXME: Arguments may be passed in the wrong order (fileSchedCycle vs fileSchedOrder)
  newInstPtr = new SchedInstruction(instNum, instName, instType, opCode,
    ...
    // FIXME: This version of new never returns nullptr - it will abort() if out of memory
...
InstCount DataDepSubGraph::CmputTwoInstDynmcLwrBound_() {
  ...
  InstCount ltncyLB = 1;
  assert(!inst2->IsRcrsvScsr(inst1) &&
         !inst1->IsRcrsvPrdcsr(inst2));
...
InstSchedule::InstSchedule(MachineModel *machMdl, DataDepGraph *dataDepGraph,
  ...
  if (instInSlot_ == nullptr || slotForInst_ == nullptr || spillCosts_ == nullptr ||
      peakRegPressures_ == nullptr) {
    Logger::Fatal("Out of memory.");
  }
